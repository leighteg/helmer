...
impl RenderGraphExecutor {
    pub fn execute(
        ...
        let batch_size_hint = encoder_batch_size.max(1);
        let mut command_buffers = Vec::with_capacity(
            compilation
                .sorted_nodes
                .len()
                .saturating_add(batch_size_hint.saturating_sub(1))
                / batch_size_hint,
        );
        ...
        for (level_idx, level) in compilation.levels.iter().enumerate() {
            if level.is_empty() {
                continue;
            }
            ...
            while start < level.len() {
                ...
                let end = (start + batch_size).min(level.len());
                let label = format!("RenderGraph/Encoder/L{level_idx}-B{}", start / batch_size);
                ...
                for node_id in &level[start..end] {
                    ...
                    if let Some(stats) = exec_stats.as_mut() {
                        if enabled {
                            stats.pass_execute_us =
                                stats.pass_execute_us.saturating_add(duration_us);
