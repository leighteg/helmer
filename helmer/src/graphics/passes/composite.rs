...
impl CompositePass {
    ...
    pub fn inputs(&self) -> CompositeInputs {
        self.inputs
    }
    ...
    fn ensure_pipeline(&self, device: &wgpu::Device, format: wgpu::TextureFormat) {
        ...
        *self.format.write() = format;
        ...
        let bgl0 = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Composite/InputsBGL"),
            entries: &[
        ...
        let bgl2 = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Composite/CameraBGL"),
        ...
        let bgl3 = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Composite/ConstantsBGL"),
            entries: &[wgpu::BindGroupLayoutEntry {
                binding: 0,
                visibility: wgpu::ShaderStages::FRAGMENT,
                ty: wgpu::BindingType::Buffer {
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Composite/PipelineLayout"),
    ...
    fn ensure_mesh_pipeline(&self, device: &wgpu::Device, format: wgpu::TextureFormat) {
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Composite/MeshPipelineLayout"),
            bind_group_layouts: &[bgl0, bgl1, bgl2, bgl3],
            immediate_size: 0,
        });

        let shader =
            device.create_shader_module(wgpu::include_wgsl!("../shaders/composite_mesh.wgsl"));

        let pipeline = device.create_mesh_pipeline(&wgpu::MeshPipelineDescriptor {
            label: Some("Composite/MeshPipeline"),
            layout: Some(&layout),
            task: None,
            mesh: wgpu::MeshState {
                module: &shader,
...
impl RenderPass for CompositePass {
    ...
    fn execute(&self, ctx: &mut RenderGraphExecCtx) {
        ...
        let mra_view = ctx
            ...
            .cloned()
            .unwrap_or_else(|| fallback.clone());
        let depth_view = match ctx.rpctx.pool.texture_view(self.inputs.depth) {
            Some(v) => v.clone(),
            None => return,
        };
        let sky_view = ctx
        ...
        let bg3 = ctx.device().create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Composite/ConstantsBG"),
        ...
        let mut pass = ctx.encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("RenderGraph/Composite"),
