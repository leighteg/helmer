...
@group(1) @binding(1) var<uniform> lights_buffer: array<LightData, MAX_WEBGL_LIGHTS>;
...
@group(2) @binding(1) var scattering_lut: texture_2d_array<f32>;
...
fn write_vertex(index: u32) {
    ...
    mesh_output.vertices[index].clip_position = vec4<f32>(x, y, 0.0, 1.0);
}
...
fn fs_main(@builtin(position) frag_coord: vec4<f32>) -> LightingOutput {
    ...
    let depth = textureLoad(depth_texture, vec2<i32>(frag_coord.xy), 0).x;
    ...
    if constants.shade_mode != 1u {
        ...
        if constants.skylight_contribution == 1u { // FULL (PBR)
            ...
            // Slightly bias normals upward for a painterly look
