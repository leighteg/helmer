...
@group(2) @binding(1) var pbr_sampler: sampler;
...
fn trace_shadow(
    ...
    loop {
        ...
        var alpha = mat.albedo.a;
        if use_textures && mat.albedo_idx >= 0i {
            let albedo_sample =
                textureSampleLevel(textures[mat.albedo_idx], pbr_sampler, hit.uv, 0.0);
            alpha = alpha * albedo_sample.a;
        }
                ...
                loop {
                    ...
                    if bnode.index_count > 0u {
                        for (var j = 0u; j < bnode.index_count; j = j + 1u) {
                            ...
                            let res = intersect_triangle(local_origin, local_dir, tri_data.p0, tri_data.p1, tri_data.p2);
                            if res.x > 0.0 && res.x < best_t {
                                best_t = res.x;
                                let w = 1.0 - res.y - res.z;
                                let uv = tri.uv0 * w + tri.uv1 * res.y + tri.uv2 * res.z;
                                let face_local = normalize(cross(tri_data.p1 - tri_data.p0, tri_data.p2 - tri_data.p0));
...
fn trace(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let dims = vec2<f32>(f32(constants.width), f32(constants.height));
    let pixel = vec2<f32>(f32(gid.x), f32(gid.y));

    let noise_dim = textureDimensions(blue_noise_tex);
    let noise_uv = vec2<i32>(i32(gid.x % noise_dim.x), i32(gid.y % noise_dim.y));
    let blue = textureLoad(blue_noise_tex, noise_uv, 0).xy;
    ...
    var sample = 0u;
    loop {
        if sample >= max(1u, constants.samples_per_frame) {
            break;
        }
        var sample_radiance = vec3<f32>(0.0);
        let jitter = vec2(rand(&seed), rand(&seed)) + blue;
        let uv = (pixel + jitter) / dims;
        let ndc = vec4(uv * 2.0 - 1.0, 1.0, 1.0);
        let world = camera.inverse_view_projection_matrix * ndc;
        let world_pos = world.xyz / world.w;
        var origin = camera.view_position;
        var dir = normalize(world_pos - origin);

        var throughput = vec3<f32>(1.0);
        let max_bounce = max(1u, constants.max_bounces);
        let max_skip = max(1u, constants.transparency_max_skip);
        var bounce = 0u;
        var transparent_skips = 0u;
        loop {
            if bounce >= max_bounce {
                break;
            }
            let hit = trace_primary(origin, dir, 1.0e30);
            if hit.hit == 0u {
                sample_radiance = sample_radiance + throughput * sample_environment(dir);
                break;
            }
            let hit_pos = hit.pos;
            let normal = hit.normal;
            var safe_normal = select(normalize(normal), vec3(0.0, 1.0, 0.0), length(normal) < 1.0e-3);
            ...
            if use_textures {
                ...
                if mat.metallic_roughness_idx >= 0i {
                    let mra_sample = textureSampleLevel(
                        textures[mat.metallic_roughness_idx],
                        pbr_sampler,
                        hit.uv,
                        0.0,
                    );
                ...
                if mat.emission_idx >= 0i {
                    let emission_sample =
                        textureSampleLevel(textures[mat.emission_idx], pbr_sampler, hit.uv, 0.0);
            ...
            if alpha_mode == ALPHA_MODE_MASK && alpha < max(cutoff, alpha_min) {
                transparent_skips = transparent_skips + 1u;
                if transparent_skips >= max_skip {
                    break;
                }
                origin = hit_pos + dir * ray_bias;
                continue;
            }
            ...
            if use_textures && mat.normal_idx >= 0i {
                ...
                let n_sample =
                    textureSampleLevel(textures[mat.normal_idx], pbr_sampler, hit.uv, 0.0).xyz
                    * 2.0
                    - 1.0;
            ...
            let alpha_ggx = max(roughness * roughness, 1.0e-4);
            let f0 = mix(vec3<f32>(0.04), albedo, metallic);
            let diffuse_color = albedo * (1.0 - metallic) * ao;

            sample_radiance = sample_radiance + throughput * emission;
            ...
            if use_direct && constants.light_count > 0u {
                ...
                loop {
                    ...
                    let shadowed = use_shadows
                        && trace_shadow(
                            hit_pos + shading_normal * shadow_bias,
                            L,
                            max_t,
                            use_textures,
                            &seed,
                    ...
                    if NdotL > 0.0 && !shadowed {
                        ...
                        let D = distribution_ggx(NdotH, alpha_ggx);
            ...
            sample_radiance = sample_radiance + throughput * direct;

            if NdotV <= 0.0 {
                break;
            }

            let spec_weight = luminance(f0);
            let diff_weight = luminance(diffuse_color);
            ...
            if rand(&seed) < spec_prob {
                let r1 = rand(&seed);
                let r2 = rand(&seed);
                let h_local = sample_ggx(r1, r2, alpha_ggx);
                let H = normalize(
                ...
                let G = geometry_smith(NdotV, NdotL, roughness);
                let spec = (D * G * F) / max(4.0 * NdotV * NdotL, EPSILON);
                let pdf = ggx_pdf(NdotH, VdotH, alpha_ggx);
                throughput = throughput * spec * (NdotL / max(pdf, EPSILON)) / max(spec_prob, EPSILON);
                dir = L;
            } else {
                let r1 = rand(&seed);
                let r2 = rand(&seed);
                ...
                throughput = throughput * diff * (NdotL / pdf) / diff_prob;
                dir = L;
            }
    ...
    radiance = radiance / f32(max(1u, constants.samples_per_frame));
    radiance = radiance * constants.exposure;

    let prev = textureLoad(history_tex, vec2<i32>(i32(gid.x), i32(gid.y)), 0);
    if constants.reset != 0u {
        textureStore(accum_tex, vec2<i32>(i32(gid.x), i32(gid.y)), vec4(radiance, 1.0));
    } else {
        let denom = f32(constants.accumulation_frame + 1u);
        let blended = (prev.xyz * f32(constants.accumulation_frame) + radiance) / denom;
        textureStore(accum_tex, vec2<i32>(i32(gid.x), i32(gid.y)), vec4(blended, 1.0));
    }
}
