...
fn trace(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    loop {
        ...
        loop {
            ...
            var surface_alpha = clamp(mat.albedo.a, 0.0, 1.0);
            let alpha_mode = mat.alpha_mode;
            let cutoff =
                clamp(mat.alpha_cutoff * constants.alpha_cutoff_scale + constants.alpha_cutoff_bias, 0.0, 1.0);
            let alpha_min = clamp(constants.alpha_cutoff_min, 0.0, 1.0);
            if alpha_mode == ALPHA_MODE_MASK && surface_alpha < max(cutoff, alpha_min) {
            ...
            if alpha_mode == ALPHA_MODE_BLEND
                ...
                if surface_alpha <= alpha_min || rand(&seed) > surface_alpha {
                    transparent_skips = transparent_skips + 1u;
                    if transparent_skips >= max_skip {
                        break;
                    }
            ...
            if use_direct && constants.light_count > 0u {
                ...
                loop {
                    ...
                    let shadowed =
                        use_shadows && trace_shadow(hit_pos + safe_normal * shadow_bias, L, max_t, &seed);
