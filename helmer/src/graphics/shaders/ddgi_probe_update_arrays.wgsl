...
fn evaluate_direct_lighting(
    ...
    loop {
        ...
        if n_dot_l > 0.0 {
            var shadowed = false;
            if use_shadows {
                shadowed = trace_shadow(pos + L * constants.shadow_bias, L, max_t, use_textures, seed);
            }
            if !shadowed {
        ...
        if dist <= ddgi.max_distance {
            ...
            if use_textures {
                if mat.metallic_roughness_idx >= 0i {
                    let mra_sample =
                        textureSampleLevel(mra_textures, pbr_sampler, hit.uv, layer_i, 0.0);
                    ao = ao * mra_sample.r;
            ...
            if use_textures && mat.normal_idx >= 0i {
                ...
                safe_normal = normalize(t * n_tangent.x + b * n_tangent.y + safe_normal * n_tangent.z);
            }
            ...
            let direct = select(
                vec3<f32>(0.0),
                evaluate_direct_lighting(hit.pos, safe_normal, use_textures, &seed),
                use_direct,
            );
