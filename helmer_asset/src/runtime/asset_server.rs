...
fn web_worker_thread_count(worker_queue_capacity: usize) -> usize {
    ...
    if let Some(max_threads) = web_worker_max_override_count() {
...
#[cfg(target_arch = "wasm32")]
fn web_worker_max_override_count() -> Option<usize> {
    let global = js_sys::global();
    let value = Reflect::get(&global, &JsValue::from_str("HELMER_WEB_WORKERS_MAX")).ok()?;
    let count = value.as_f64()?;
    if count.is_finite() && count >= 1.0 {
        Some(count.floor() as usize)
    } else {
...
#[cfg(not(target_arch = "wasm32"))]
fn configured_asset_worker_count(worker_queue_capacity: usize) -> usize {
    if let Ok(value) = std::env::var("HELMER_ASSET_WORKERS")
        && let Ok(parsed) = value.parse::<usize>()
        && parsed > 0
    {
        return if worker_queue_capacity > 0 {
            parsed.min(worker_queue_capacity)
        } else {
            parsed
        };
    }
    ...
    let parallelism = std::thread::available_parallelism()
        ...
        .unwrap_or(1)
        .max(1);
    let mut workers = if runtime_single_thread_override() {
        1
    } else {
        parallelism.saturating_sub(1).max(1)
    };
    if worker_queue_capacity > 0 {
        workers = workers.min(worker_queue_capacity);
    }
    workers.max(1)
}
...
impl AssetServer {
    pub fn new(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            let num_workers = configured_asset_worker_count(worker_queue_capacity);
