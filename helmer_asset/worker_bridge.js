...
const WORKER_CONFIG_KEYS = [
  "HELMER_WASM_MESHOPT",
  "HELMER_WASM_MESHOPT_VCACHE",
  "HELMER_WASM_MESHOPT_SIMPLIFY",
  "HELMER_WASM_MESHLETS",
];
...
function collectWorkerConfig() {
  const config = {};
  for (const key of WORKER_CONFIG_KEYS) {
    const normalized = normalizeBool(GLOBAL_SCOPE[key]);
...
function applyWorkerConfig(worker) {
  const config = collectWorkerConfig();
  if (!config || Object.keys(config).length === 0) {
    return;
  }
  worker.postMessage({ type: "config", values: config });
}

function broadcastWorkerConfig() {
