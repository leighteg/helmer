...
use crate::graphics::{
    common::{
        ...
        graph::{
            RenderGraphBuildOutput, RenderGraphBuildParams, RenderGraphSpec,
...
#[derive(Default)]
pub struct BuiltinRenderGraphTemplateCatalog;

impl RenderGraphTemplateCatalog for BuiltinRenderGraphTemplateCatalog {
    fn default_spec(&self) -> RenderGraphSpec {
        default_graph_spec()
    }

    fn template_spec(&self, name: &str) -> Option<RenderGraphSpec> {
        template_for_graph(name).map(|template| (template.build)())
    }
}

pub fn builtin_graph_template_catalog() -> Arc<dyn RenderGraphTemplateCatalog> {
    Arc::new(BuiltinRenderGraphTemplateCatalog)
}
...
fn build_default_graph(
    ...
    if toggles.occlusion_culling && params.device_caps.supports_compute_storage_textures() {
