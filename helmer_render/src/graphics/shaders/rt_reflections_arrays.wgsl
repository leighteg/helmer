...
@group(3) @binding(0) var albedo_textures: texture_2d_array<f32>;
@group(3) @binding(1) var normal_textures: texture_2d_array<f32>;
@group(3) @binding(2) var mra_textures: texture_2d_array<f32>;
@group(3) @binding(3) var emission_textures: texture_2d_array<f32>;
@group(3) @binding(4) var pbr_sampler: sampler;
...
fn trace_shadow(
    ...
    loop {
        ...
        if use_textures && mat.albedo_idx >= 0i {
            ...
            let layer = select(hit.material_id, 0u, hit.material_id >= layer_limit);
...
fn evaluate_direct_lighting(
    ...
    loop {
        ...
        if n_dot_l > 0.0 {
            ...
            if use_shadows {
                shadowed =
                    trace_shadow(pos + normal * constants.shadow_bias, L, max_t, use_textures, seed);
            }
            if !shadowed {
                ...
                let D = distribution_ggx(n_dot_h, alpha_ggx);
...
fn rt_reflections(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let use_textures = (flags & RT_FLAG_USE_TEXTURES) != 0u;
    ...
    let alpha_ggx = max(surface_roughness * surface_roughness, 1.0e-4);
    ...
    loop {
        ...
        var origin = world_pos + normal * ray_bias;
        var hit = trace_primary(origin, dir, 1.0e30);
        ...
        loop {
            ...
            if alpha_mode == ALPHA_MODE_MASK && alpha < max(cutoff, alpha_min) {
                ...
                hit = trace_primary(origin, dir, 1.0e30);
                continue;
            }
            ...
            if use_textures && mat.normal_idx >= 0i {
                ...
                shading_normal =
                    normalize(t * n_tangent.x + b * n_tangent.y + safe_normal * n_tangent.z);
            }
