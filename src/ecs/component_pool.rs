...
impl ComponentPool {
        ...
        T2: Component + 'static,
    {
        ...
        let comp1 = map1.unwrap().get_mut(&entity)?.as_any_mut().downcast_mut::<T1>()?;
        let comp2 = map2.unwrap().get_mut(&entity)?.as_any_mut().downcast_mut::<T2>()?;
    ...
    pub fn query_mut_three<'a, T1, T2, T3>(&'a mut self, entity: Entity) -> Option<(&'a mut T1, &'a mut T2, &'a mut T3)>
    where
        T1: Component + 'static,
        T2: Component + 'static,
        T3: Component + 'static,
    {
        ...
        let [map1, map2, map3] = self.components.get_many_mut([&type_id1, &type_id2, &type_id3]);
        
        let comp1 = map1.unwrap().get_mut(&entity)?.as_any_mut().downcast_mut::<T1>()?;
        let comp2 = map2.unwrap().get_mut(&entity)?.as_any_mut().downcast_mut::<T2>()?;
        let comp3 = map3.unwrap().get_mut(&entity)?.as_any_mut().downcast_mut::<T3>()?;
...
impl ComponentPool {
    ...
    pub fn get_entities_with_exactly(&self, type_ids: &[TypeId]) -> Vec<Entity> {
        ...
        for entity in candidates {
            ...
            let entity_types: HashSet<TypeId> = self.components
                .iter()
